...
export function ExpenseCategoryManager({ categories }: { categories: ExpenseCategoryRow[] }) {
  ...
  return (
    <div className="mb-6 rounded border border-rule bg-paper-card shadow-card p-3">
      ...
      {categories.length > 0 && (
        <ClearAllDataButton
          label="Clear all expense data"
          confirmMessage={`This permanently deletes all ${categories.length} categor${
            categories.length === 1 ? "y" : "ies"
          } and every month of amounts ever entered or imported under them — not just this month. This cannot be undone.`}
          onConfirm={clearAllExpenseData}
        />
      )}
    </div>
  );
